...
Gameplay now rejects component overlap on normal click placement, supports Delete/Backspace removal of the active board part, supports Ctrl+D duplication into the nearest open grid location, and bounds board history to 96 snapshots. A source-derived power-network topology snapshot now reports source-reachable parts, isolated parts, valid wire links, and source-reachable LEDs; it is explicitly a gameplay/diagnostic graph and does not replace SPICE or validated electrical quantities. Visual presentation adds a text-free half-unit PCB placement grid, a thin board-depth underlay, softer secondary rim lighting, and responsive stretch behavior while keeping the PCB itself free of labels.
