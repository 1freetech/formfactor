...
bool valid_sha256_hex(std::string_view digest) {
...
std::optional<std::string> sha256_hex(std::string_view bytes) {
  ...
  if (bytes.size() > maximum_bytes) return std::nullopt;

  std::vector<std::uint8_t> message;
  if (bytes.size() > message.max_size() - 72U) return std::nullopt;
  message.reserve(bytes.size() + 72U);
  for (const unsigned char byte : bytes) message.push_back(byte);
  ...
  for (std::size_t block = 0; block < message.size(); block += 64U) {
    ...
    for (std::size_t index = 0; index < words.size(); ++index) {
      ...
      const auto temporary1 =
          h + big_sigma1 + choice + round_constants[index] + words[index];
  ...
  for (const auto word : state) {
    for (int shift = 28; shift >= 0; shift -= 4) {
      digest.push_back(hexadecimal[(word >> shift) & 0x0fU]);
    }
  }
  return digest;
}
