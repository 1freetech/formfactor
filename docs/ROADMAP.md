...
- Exact unit-aware quantities. ([sourced catalogue-property records and exact numeric filtering complete](CATALOG_QUANTITY_PROPERTIES.md); explicit schemas cover all four qualifiers for the nine currently supported non-dimensionless dimensions; conservative component-family applicability, exact property-to-part identity, source-artifact digest, exact in-artifact locator, and exact source-claim text, canonical HTTPS source-URL, and exact publisher-origin policy and policy-artifact provenance, exact SHA-256 byte verification, and Ed25519 signing-key fingerprint and detached-signature envelope validation, and OpenSSL-backed cryptographic detached-signature verification and deterministic pinned-key trust-root evaluation and exact N-to-N+1 root transition precheck gates complete; signed trust-root distribution, dual-threshold signature verification, durable persistence, threshold rotation and revocation, catalogue export integration, policy retrieval, publisher authentication, and semantic transcription verification pending)
