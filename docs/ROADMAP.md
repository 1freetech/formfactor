...
- Exact unit-aware quantities. ([sourced catalogue-property records and exact numeric filtering complete](CATALOG_QUANTITY_PROPERTIES.md); explicit schemas cover all four qualifiers for the nine currently supported non-dimensionless dimensions; conservative component-family applicability, exact property-to-part identity, source-artifact digest, exact in-artifact locator, and exact source-claim text, canonical HTTPS source-URL, and exact publisher-origin policy gates complete; policy provenance, catalogue export integration, publisher authentication, and semantic transcription verification pending)
