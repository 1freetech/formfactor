...
6. Source metadata requires visible single-line title and revision text plus a URL in FormFactor's conservative absolute-HTTPS profile. The profile requires lowercase `https://`, a lowercase ASCII DNS host with optional numeric port, no credentials, no fragment, and no whitespace or control characters. It is based on the URI structure defined by RFC 3986 but intentionally narrower; syntactic acceptance does not authenticate the host, authorize a publisher, fetch the resource, or verify transcription.
...
6. rejection of duplicate or malformed IDs, unsupported qualifiers, empty/control-bearing labels and conditions, incomplete/control-bearing source metadata, and missing, whitespace-only, or control-bearing in-artifact locators;
...
- A structurally valid HTTPS URL, source record, artifact digest, in-artifact locator, and preserved claim text are not proof of publisher authenticity or authorization, locator correctness, semantic transcription correctness, or measurement accuracy.
