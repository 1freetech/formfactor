...
## Pinned policy-key trust roots

CR-020 evaluates an explicitly supplied, versioned key-trust root at an explicitly supplied UTC instant. The fixed `YYYY-MM-DDTHH:MM:SSZ` timestamp profile, root version, expiry, exact publisher ID, and complete CR-017-validated key identity are required. An exact binding in an unexpired root produces `Trusted`; unknown, changed, or removed bindings produce `NotTrusted`; evaluation at or after expiry produces `Expired`. Malformed timestamps, duplicate publisher/key identities, zero versions, empty roots, and invalid keys fail closed as `Invalid` without a record.

The deterministic `formfactor-policy-key-trust-v1` record sorts bindings and preserves SHA-256 digests of validated key records rather than raw public-key bytes. The design follows the Update Framework's separation between a client-shipped trusted root, versioned/expiring root metadata, and trust removal by omitting a key from a newer root. It is not a TUF implementation. FormFactor does not yet authenticate or distribute the supplied root, prevent rollback to an older version, verify threshold root rotation, retain trusted state, or connect this decision to catalogue export.


## Root update sequence precheck

CR-021 checks the rollback-sensitive structural transition from a currently trusted root version N to a candidate version exactly N+1. A lower or repeated version produces `Rollback`; a forward gap produces `VersionGap`; a different root identity produces `DifferentRoot`; and a candidate that is expired at the fixed evaluation instant produces `Expired`. The current root may be expired while an update is attempted, matching the Update Framework workflow, but the final candidate must not be expired. Invalid roots, timestamps, ambiguous keys, and version-counter overflow fail closed without a record.

Deterministic `formfactor-policy-key-root-transition-v1` evidence binds both validated root records and the evaluation instant without embedding raw keys. Every record explicitly states that signature verification and persistence were not performed. A `Sequential` result is only permission to continue validation; it is not acceptance of the candidate root. The required old-root and new-root signature thresholds, unique-key counting, non-volatile persistence, trusted-state loading, retrieval limits, and catalogue integration remain unimplemented.
