...
1. `make_quantity` now rejects every `Unit` identifier not explicitly listed by formfactor.
2. `make_si_quantity` now rejects every `Dimension` identifier not explicitly listed by formfactor.
...
5. Both factories return an empty `std::optional` at the import boundary. The caller must preserve the value as unknown or report an error; formfactor does not guess the intended unit.
...
Prerequisite: open a terminal in the formfactor repository with GCC installed.
