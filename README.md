...
The game-facing design also follows the [Gameplay, Graphics, Presentation, and UX Benchmarks](docs/GAMEPLAY_UX_BENCHMARKS.md). That contract now combines lessons from highly rated games, open-world sandbox design, creator platforms, and sports-simulation training systems: an always-available FreeLab sandbox, multi-stage Engineering Contracts, reproducible checkpoints, context-preserving view switching, editable working templates, reusable scenario modules, rapid creator playtesting, form-factor preview, Lite-mode parity, accessibility, focused practice drills, adjustable assistance that never changes engineering truth, evidence-backed action feedback, targeted practice recommendations, mastery tracking, and presentation streaming that never weakens engineering truth. The detailed [GTA and Open-World Design Research](docs/GTA_OPEN_WORLD_RESEARCH.md), [Fortnite and Roblox Creator-System Research](docs/FORTNITE_ROBLOX_CREATOR_RESEARCH.md), and [MLB The Show Design Research](docs/MLB_THE_SHOW_DESIGN_RESEARCH.md) record the source-specific lessons and license guardrails. Proprietary game assets, missions, branding, platform code, broadcast packages, player likenesses, and reverse-engineered GTA source are not copied into FormFactor.
