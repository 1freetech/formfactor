...
**Current frontend concentration: gameplay feel.** FormFactor should be realistic underneath but easy and satisfying to control. The next production work prioritizes immediate input response, simple select/place/move/rotate/connect/test controls, placement and connection previews, safe cancel, undo/redo, fast retries, clear evidence feedback, and a smooth **Place -> Connect -> Power -> Test -> Diagnose -> Repair -> Validate** loop before broad 3D or visual expansion. See [Gameplay Feel Requirements](docs/GAMEPLAY_FEEL_REQUIREMENTS.md).

The game-facing design also follows the [Gameplay, Graphics, Presentation, and UX Benchmarks](docs/GAMEPLAY_UX_BENCHMARKS.md) and the [Latest FormFactor Game Design Update](docs/LATEST_GAME_DESIGN_UPDATE.md). Current research combines lessons from highly rated games, GTA, Fortnite, Roblox, MLB The Show, NBA 2K, Spore, Space Engineers, Stationeers, Satisfactory, and extreme-scale worlds. The resulting direction includes FreeLab, multi-stage Engineering Contracts, reproducible checkpoints, focused practice, evidence-backed feedback, a scalable searchable component library, Favorites/Recent, quick slots, component filters, an engineering Details/Codex view, Design Scouting Reports, Project Evolution Timelines, reusable Engineering Blueprints, repair-state gameplay, creator modules, safe future scripting, large-project navigation, project-region streaming, LOD, and 2D Lite/full-presentation parity without weakening engineering truth. The component inventory direction is detailed in [Inventory and Component Library Design](docs/INVENTORY_COMPONENT_LIBRARY_DESIGN.md). Additional research is recorded in [GTA and Open-World Design Research](docs/GTA_OPEN_WORLD_RESEARCH.md), [Fortnite and Roblox Creator-System Research](docs/FORTNITE_ROBLOX_CREATOR_RESEARCH.md), [MLB The Show Design Research](docs/MLB_THE_SHOW_DESIGN_RESEARCH.md), and [NBA 2K, Spore, Space Engineers, and Extreme-Scale World Research](docs/NBA2K_SPORE_SPACE_SCALE_RESEARCH.md). Proprietary game assets, maps, missions, branding, platform code, broadcast packages, player likenesses, and reverse-engineered GTA source are not copied into FormFactor.

## FormFactor 1.127 — Pin Graph

FormFactor now connects **pins, not just whole parts**. A wire remembers exact endpoints such as `PWR1.POS -> R1.1` or `D1.K -> PWR1.NEG`. The live schematic shows those pin names, the editor groups connected pins into named nets, and saved Engineering Blueprints keep the exact pin wiring when they are restored.

The digital core now supports NOT, AND, OR, NAND, NOR, XOR, and XNOR with deterministic truth-table generation. A new KiCad legacy-schematic importer preserves explicit reference, symbol, value, and footprint fields. Placed parts can also carry symbol -> footprint -> package metadata, while reference-only and visual-only data stays clearly labeled instead of being treated as verified engineering truth.

The compact 1.127 toolbar adds SELECT, PLACE, WIRE, MOVE, ROTATE, INSPECT, TEST, TRUTH, SAVE BP, and LOAD BP flows. TEST now performs a pin-level topology precheck for a simple power/LED loop, then clearly stops short of claiming electrical validity. The editor exports a versioned engineering snapshot that says `cpp_core_required`, so future solver bridges can consume what the player actually built without letting graphics decide whether a circuit works. See [FormFactor 1.127 Open-Source Integration](docs/OPEN_SOURCE_REPO_INTEGRATION_1127.md) and [Third-Party Notices](THIRD_PARTY_NOTICES.md).
