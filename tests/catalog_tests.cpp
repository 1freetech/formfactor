...
pcbtech::Source property_source() {
  return {"Synthetic datasheet fixture",
          "https://example.invalid/property", "Rev A"};
}

pcbtech::Quantity quantity(std::int64_t coefficient, int decimal_exponent,
                           pcbtech::Unit unit) {
  const auto value =
      pcbtech::make_quantity(coefficient, decimal_exponent, unit);
  assert(value.has_value());
  return *value;
}

pcbtech::CatalogQuantityProperty property(
    const char* id, const char* name, pcbtech::Quantity value,
    pcbtech::QuantityValueQualifier qualifier,
...
int main() {
  ...
  pcbtech::CatalogEntry entry{
      ...
      {property("voltage.maximum", "Maximum voltage",
                quantity(16, 0, pcbtech::Unit::Volt),
                pcbtech::QuantityValueQualifier::Maximum),
       property("capacitance.nominal", "Nominal capacitance",
                quantity(1, 0, pcbtech::Unit::Microfarad),
                pcbtech::QuantityValueQualifier::Nominal,
                "at stated test conditions")}};

  const auto valid = pcbtech::validate_catalog_entry(entry);
  ...
  const auto localized_record =
      pcbtech::validate_catalog_entry(entry).canonical_quantity_property_record;
  ...
  assert(pcbtech::validate_catalog_entry(reversed)
             .canonical_quantity_property_record ==
  ...
  one_volt.quantity_properties = {
      property("voltage.nominal", "Nominal voltage",
               quantity(1, 0, pcbtech::Unit::Volt),
               pcbtech::QuantityValueQualifier::Nominal)};
  auto thousand_millivolts = one_volt;
  thousand_millivolts.quantity_properties.front().value =
      quantity(1000, 0, pcbtech::Unit::Millivolt);
  assert(pcbtech::validate_catalog_entry(one_volt)
             .canonical_quantity_property_record ==
         pcbtech::validate_catalog_entry(thousand_millivolts)
             .canonical_quantity_property_record);
  ...
  const auto missing = pcbtech::validate_catalog_entry(no_properties);
  ...
  conditioned.quantity_properties.front().conditions =
      "at stated test conditions";
  const auto unconditional_record = pcbtech::validate_catalog_entry(one_volt)
                                        .canonical_quantity_property_record;
  const auto conditioned_record = pcbtech::validate_catalog_entry(conditioned)
                                      .canonical_quantity_property_record;
  ...
  const std::array qualifier_cases{
      std::pair{pcbtech::QuantityValueQualifier::Nominal, "nominal"},
      std::pair{pcbtech::QuantityValueQualifier::Minimum, "minimum"},
      std::pair{pcbtech::QuantityValueQualifier::Typical, "typical"},
      std::pair{pcbtech::QuantityValueQualifier::Maximum, "maximum"}};
  for (const auto& [qualifier, name] : qualifier_cases) {
    ...
    const auto result = pcbtech::validate_catalog_entry(qualified);
  ...
  const auto expect_rejected = [](const pcbtech::CatalogEntry& invalid) {
    const auto first = pcbtech::validate_catalog_entry(invalid);
    const auto replay = pcbtech::validate_catalog_entry(invalid);
  ...
  for (const int raw : {-1, 4, std::numeric_limits<int>::max()}) {
    ...
    invalid.quantity_properties.front().qualifier =
        static_cast<pcbtech::QuantityValueQualifier>(raw);
