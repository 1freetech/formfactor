...
int main() {
  using pcbtech::DigitalEvent;
  using pcbtech::DigitalGate;
  using pcbtech::GateType;
  using pcbtech::LogicState;

  const std::vector<DigitalGate> inverter_chain{
      {"INV1", GateType::Not, {"A"}, "B", 5},
      {"INV2", GateType::Not, {"B"}, "C", 7}};
  const auto chain = pcbtech::simulate_digital(inverter_chain, {{0, "A", LogicState::High}});
  ...
  const auto dominant_low = pcbtech::simulate_digital(
  ...
  const auto unknown_output = pcbtech::simulate_digital(
