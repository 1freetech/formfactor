...
int main() {
  ...
  assert(!formfactor::validate(missing_model).export_allowed());
  ...
  for (const auto tier : tiers) {
    for (std::size_t i = 0; i < rating_fields.size(); ++i) {
      ...
      for (const auto value : invalid_values) {
        ...
        const auto result = formfactor::validate(candidate);
        assert(!result.export_allowed());
        assert(result.errors == std::vector<std::string>{rating_errors[i]});
        const auto replay = formfactor::validate(candidate);
  ...
  for (const auto tier : tiers) {
    for (const auto field : rating_fields) {
      for (const auto value : positive_values) {
        ...
        assert(result.export_allowed() == (tier == formfactor::AccuracyTier::Verified));
  ...
  const auto result = formfactor::validate(all_invalid);
  const std::vector<std::string> expected_errors(rating_errors.begin(),
                                                rating_errors.end());
  assert(result.errors == expected_errors);
  assert(!result.export_allowed());
  assert(formfactor::validate(all_invalid).errors == expected_errors);
  ...
  assert(!formfactor::validate(formfactor::Component{}).export_allowed());
}
